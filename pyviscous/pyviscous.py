...
import copulae
from copulae import GaussianMixtureCopula  # used to utilize GMCM more flexibly
...
# ########## Data Overview ###########
# (x, y): Original input-output data.
...
def viscous(x, y, xIndex, sensType, N1=2000, N2=2000, n_components='optimal', MSC='AIC', verbose=False):
    ...
    # Note: Part B (GMCM inference) and Part C (Sensitivity index computation) are all based on the normalized data (ie, x_sens_norm and y_norm).
    ...
    while trial_id <= trial_max:
        ...
        except AttributeError: # if gmcm_inference does not succeed or gmcm_params is null.
            trial_id += 1 
        ...
        # Check if sens_indx is a valid result (not NaN).
    ...
    # Possible reasons include inappropriate input arguments, under-dispersed x and y data, or GMCM non-convergence.
    if sens_indx>1:
        sens_indx = 1
...
def gmcm_inference(x_norm, y_norm, n_components, MSC, verbose):
    ...
    n_xvariables = x_norm.shape[1]

    if isinstance(n_components, str) and n_components=='optimal':
        n_components = list(np.arange(1, 10)) # Multiple candidate GMCMs. 
                                              # Hard coded to limit the number of GMCM parameters. 
    elif isinstance(n_components, int) and n_components>0:
        n_components = [n_components]
    else:
        raise ValueError('The provided n_components does not meet the requirement: '
                         'a string "optimal" or an integer greater than zero.')
    ...
    bics = []
    aics = []
    n_components_converged = []

    # Start n_components loops
    for n_component in n_components:        
        ...
        for trial_id in range(1, 4):
            ...
            try:
                ...
                if verbose:
                    print(f'\tinitial {trial_id}: fitting fails due to k_means initialization ValueError.')
                continue

            # Fit the GMCM's params 
            try:
                ...
                try:
                    # Get convergence label, convg_label
                    convg_label = getattr(g_cop_gmcm._fit_smry, "has_converged")
                    if convg_label:
                        if verbose:
                            print(f'\tinitial {trial_id}: fitting completes with convergence.')
                        break
                    trial_id += 1
                except:
                    if verbose:
                        print(f'\tinitial {trial_id}: fitting completes until max iterations. Does not converge.')
                    continue
            except:
                if verbose:
                    print(f'\tinitial {trial_id}: fitting fails due to singular matrix error.')
                continue                

        # ---------------------------------
        # Compute the model selection score for the fitted GMCM.  
        # Note: only record converged GMCMs.  
        try:
            # Get convergence label: convg_label
            convg_label = getattr(g_cop_gmcm._fit_smry, "has_converged")
            ...
            # Calculate BIC/AIC for the fitted GMCM 
            ...
            n_components_converged.append(n_component)
    
        except AttributeError:
            if verbose:
                print(f"\tn_component = {n_component}, does not converge.")
            continue    
    # End n_components loops
    
    # Identify the best fitted GMCM.
    if n_components_converged:  # if the best fitted GMCM is not empty.
        if MSC == 'aic':
            gmcm_model_comparisons = pd.DataFrame({"n_components": n_components_converged, "MSC": aics})
        elif MSC == 'bic':
            gmcm_model_comparisons = pd.DataFrame({"n_components": n_components_converged, "MSC": bics})

        # Identify the minimum AIC/BIC score corresponding index.
...
def sensivitiy_index_estimation(sensType, gmcm, N1, N2, varY, cdf_y_function):    
    ...
    for r1 in range(N1):
        ...
        # Replace x of z2_MC with x of the r1^th sample from z1_MC
        z2_MC[:,:-1] = np.ones_like(z2_MC[:,:-1])*z1_MC[r1,:-1] 

        # Compute F^(-1)(zy2_MC), marginal CDF corresponding y values in its original space.
...
def k_means(data: np.ndarray, n_clusters: int, n_dim: int, ties='average', init='random'):
    ...
    GMCParam = FitSummary(GMCParam(n_clusters, n_dim, prob, means, covs), True, 'kmeans', len(data_cdf),
...
def gmm_marginal_cdf(x, param):
    """
    Calculate the marginal cdf for the input x given the GMCM parameters.
    ...
    cdf_values = np.where(zi < 0, erf, 1 - erf) @ param.prob
    return cdf_values